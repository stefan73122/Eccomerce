...
export default function AddAddressPage() {
  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setErrors({ fullAddressLine: 'Error al guardar la dirección. Intenta de nuevo.' });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="px-4 sm:px-6 lg:px-20 py-10 max-w-2xl mx-auto">
        <h1 className="text-[26px] font-bold text-[var(--text-dark)] mb-8">Agregar nueva dirección</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            {loadingRegions ? (
              <div className="flex items-center gap-2 h-11 px-3 border border-[var(--border)] rounded-lg">
                <Loader2 size={16} className="animate-spin text-[var(--text-muted)]" />
                <span className="text-sm text-[var(--text-muted)]">Cargando regiones…</span>
              </div>
            ) : (
              <select
                value={regionId}
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Av. Cristo Redentor #1234, entre Calle 5 y 6, Edificio Torre Azul, Piso 3, Depto 301"
              rows={3}
              className={cn(
